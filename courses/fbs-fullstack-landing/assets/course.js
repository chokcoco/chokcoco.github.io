document.addEventListener("DOMContentLoaded", () => {
  ...
  const activeChapter = document.querySelector(".chapter-nav [aria-current='page']");
  if (chapterNav && chapterNavToggle) {
    const narrow = () => window.matchMedia("(max-width: 1240px)").matches;
    const setExpanded = (expanded) => {
      chapterNav.classList.toggle("is-collapsed", !expanded && !narrow());
      chapterNav.classList.toggle("is-expanded", expanded && narrow());
      chapterNavToggle.setAttribute("aria-expanded", String(expanded));
      const label = chapterNavToggle.querySelector(".chapter-nav-toggle-label");
      if (label) label.textContent = expanded ? "收起" : "展开";
    };

    setExpanded(!narrow());
    activeChapter?.scrollIntoView({ block: "center" });
    chapterNavToggle.addEventListener("click", () => setExpanded(chapterNavToggle.getAttribute("aria-expanded") !== "true"));
    window.addEventListener("resize", () => setExpanded(!narrow()));
  }
});
