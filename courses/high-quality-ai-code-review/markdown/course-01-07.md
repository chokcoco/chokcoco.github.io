...
> 预计学习时间：80–100 分钟
> 一句话总结：把"AI 偷懒"还原为可观察的输出衰减、步骤遗漏和流程控制失效，然后从实验证据中找出真正需要解决的问题。
...
> 预计学习时间：90–110 分钟
> 一句话总结：用外部完成证明、质量门和漏报回流推动每一批审查真正结束，让漏掉的问题有路径回到系统中。
...
提升召回率的核心不是让模型"更努力"，而是让系统"更可靠"。语义分批把大任务变成模型舒适区内的小任务。状态机用外部硬约束代替模型的自我管理。质量门用可配置的阈值验证每一步的产出。扩展检查从接口、需求、记忆和历史多个角度补充发现。数据回流让每一次遗漏都变成下一次审查的知识。

这些机制共同构成了一条闭环：审查 → 验证 → 遗漏发现 → 规则提炼 → 补充审查 → 再次验证。它不是一次性的优化，而是一个持续运行的改进飞轮。

第 8 章会把这条闭环从本地扩展到远端。当审查能力变成一种服务，可以接入其他生码平台和 CI 流水线时，反馈来源会更多、规则积累会更快、召回率的提升也会更显著。

## 本章源码观察点

`qms-server` 中的以下代码路径与本章讨论直接对应：

- `batchAllocator.ts`：三阶段语义分批算法；800–1200 行理想范围；兜底 100 行拆分。
- `processor.ts`：`allowed_next_step` 状态机（0→1→2→2.x→3）；`fallbackToStep1` 降级恢复；`maxRetryCount=3`。
- `verifier.ts`：质量验证（必填字段 + 内容长度）、时间验证（`minTimeCoefficient`）、密度验证（`minDensityCoefficient` / `linesPerExpectedIssue`）三层检查；时间与密度的"或"策略。
- `extensionChecker.ts`：`CHECKER_CONFIG` 定义了 recheck(1)、yapi(2)、td(3)、memory_pos(4)、filter(98)、fix(99) 的优先级和策略；`processExtensionResults` 统一分发。
- `memoryDistiller.ts`：`distill` 流程从原始信号中提炼 negative/positive/missed 三类规则；`enabledSources` 默认 `['user_reject', 'bug']`；`minOccurrence=2`、`minConfidence=0.5`、`ruleExpireDays=90`。
- `memoryChecker.ts`：`runMemoryPosChecker` 加载正面和遗漏规则，生成补充审查 Prompt；`processMemoryPosResults` 把新 Issue 写入数据库。
- `bugSync.ts`：`judgeRecallStatus` 三层过滤（status→bugReason→labels）；`judgeOwnerRoleByContent` 角色归属判定。
- `utils/config.ts`：`extensionCheckers.enabled` 默认 `['recheck', 'filter', 'yapi', 'td', 'fix']`，不含 `memory_pos` 和 `memory_neg`。
